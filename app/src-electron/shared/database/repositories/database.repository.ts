...
> implements DatabaseInterfaceRepository<T> {
  ...
  private getRepository(): Repository<T> {
    return this.entity;
    // this.txHost?.tx?.getRepository(this.entity.target) || ;
  }
  ...
  public getRelatedEntityNames(): string[] {
    return this.getRepository()
      .metadata.relations.filter(
        (relation) => relation.isManyToOne || relation.isOneToOne || relation.isManyToMany,
      )
  ...
  public async upsert(data: Partial<T>, uniqueFields?: string[]): Promise<InsertResult> {
  ...
  public async update(id: string | number, data: QueryDeepPartialEntity<T>): Promise<T | null> {
  ...
  async updateAssociations<U extends { id: number | string } & Record<string, unknown>>({
    ...
    for (const existing of existingItems) {
      const stillExists = updatedItems.some((updated) => match(existing, updated));
      if (stillExists) {
    ...
    for (const updated of updatedItems) {
      const alreadyExists = existingItems.some((existing) => match(existing, updated));
      if (!alreadyExists) {
  ...
  async updateJunctionAssociations<U extends { id: number | string } & Record<string, unknown>>({
    existingItems,
    updatedItems,
    keys,
    onCreate,

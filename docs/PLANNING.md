# Clarix: Project Planning & Specification

## 1. Executive Summary

Clarix is an AI-native business intelligence platform that connects to any database, reads its schema, and lets users generate charts, dashboards, and analytics insights through natural language prompts — no SQL knowledge required.

## 2. Technical Specification

### 2.1 Core Features

1. **Data Source Management**: Connect and manage various database types (PostgreSQL, MySQL, etc.).
2. **AI Agent Interface**: A conversational or prompt-based interface where users request data insights.
3. **Dynamic Dashboards**: Auto-generated interactive charts (bar, line, pie, etc.) and tabular data representations based on the AI agent's SQL queries and data analysis.

### 2.2 Development Phases

#### Phase 1: Infrastructure & Data Source Management

- [ ] **Database Connection CRUD**: Implement the backend/IPC modules to manage database configurations securely.
- [ ] **Connection Testing**: Create a UI to verify database connectivity.
- [ ] **Data Source UI**: Develop a frontend interface (grid/list views) to display and edit connected databases.

#### Phase 2: AI Agent Integration

- [ ] **Agent Configuration**: Setup environment variables and LLM configurations (OpenAI, Anthropic, or local models).
- [ ] **Text-to-SQL Pipeline**: Implement the agent logic to translate user prompts into valid SQL queries targeting the connected database schema.
- [ ] **Data Retrieval**: Execute the generated SQL securely (with read-only permissions) and return the dataset.
- [ ] **Agent Testing UI**: Create a testing interface to send prompts, view generated SQL, and preview the raw data output.

#### Phase 3: Dashboard & Visualization Engine

- [ ] **Charting Library Setup**: Integrate a robust charting library (e.g., Chart.js, Recharts, or ApexCharts) compatible with the frontend framework.
- [ ] **Data-to-Chart Mapping**: Implement an abstraction layer that maps the AI-retrieved dataset to appropriate chart configurations based on the data types (e.g., time-series data to line charts).
- [ ] **Interactive Datatables**: Implement a robust table component (with sorting, filtering, and pagination) to display the raw data alongside the charts.
- [ ] **Dashboard Layout**: Create a flexible grid system allowing users to resize, drag, and arrange generated chart and table cards.

#### Phase 4: Polish & User Experience

- [ ] **Loading States & Error Handling**: Implement skeleton loaders for dashboards and descriptive error messages for failed queries or agent hallucinations.
- [ ] **Export Functionality**: Allow users to export charts as images (PNG/PDF) and tables as CSV files.
- [ ] **Theming**: Ensure consistent light/dark mode support across all UI elements and charts.

### 2.3 Technical Stack Considerations

- **Frontend**: Angular with Spartan UI and TailwindCSS.
- **Backend/IPC**: Electron main process with TypeORM.
- **AI Integration**: Python-based AI agents communicating via IPC.
- **Database (app)**: Local SQLite/PostgreSQL.

---

## 3. Business & Startup Plan
...
Clarix provides a **chat-first analytics interface** powered by LLMs:
...
| Segment                         | Pain Point                                | How Clarix Helps                                 |
| ------------------------------- | ----------------------------------------- | ------------------------------------------------ |
...
### 3.5 Business Model — Freemium + Paid Tiers
...
- **Embedded analytics add-on** — for SaaS companies embedding Clarix in their products
...
- Publish SEO content: _"How to visualize your PostgreSQL data without SQL"_
...
**Competitive Edge:** Clarix is the only fully AI-native, affordable, prompt-first BI tool targeting SMBs and developers — a segment largely ignored by enterprise players.

### 3.8 Team & Roles Needed
...
_Clarix — Built for the team that just wants answers, not another dashboard to maintain._
